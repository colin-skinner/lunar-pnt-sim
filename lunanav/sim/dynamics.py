...
def rigid_body_derivative(t: float, state: jnp.ndarray, force: jnp.ndarray, torque: jnp.ndarray,
    ...
    mass_kg : float
    I : jnp.ndarray
        Moment of inertia (body frame) [kg·m²]
...
def lander_motion_inertial(state: jnp.ndarray, force_I: jnp.ndarray, torque_B: jnp.ndarray, dt: float, mass: float, I: np.ndarray, mu: float = GM_MOON):
    ...
    force : jnp.ndarray (3,)
        Applied specific (non-gravity) force acting on the body (in body frame) [N]
...
def lander_motion(state: jnp.ndarray, force_B: jnp.ndarray, torque_B: jnp.ndarray, dt: float, mass: float, I: np.ndarray, mu: float = GM_MOON):
    ...
    return lander_motion_inertial(state, force_I, torque_B, dt, mass, I, mu)
